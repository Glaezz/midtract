 ...
 */
export async function fundBuyerWallet(orderCode) {
    const order = await db.order.findUniqueOrThrow({ where: { orderCode }, include: { buyer: true } });
    if (!order.buyer)
        throw new Error(`Order ${orderCode} belum punya buyer, tidak bisa di-fund`);
 ...
 */
export async function reclaimUnusedTopup(orderCode) {
    const order = await db.order.findUniqueOrThrow({ where: { orderCode }, include: { buyer: true } });
    if (!order.buyer)
        return; // tidak pernah di-fund karena belum ada buyer, tidak ada yang direclaim
    ...
    try {
        ...
        if (/allowance|InsufficientAllowance/i.test(msg)) {
            ...
            console.error(`[reclaim] KRITIS ${orderCode}: USDC top-up TERSELAMATKAN di wallet buyer ` +
