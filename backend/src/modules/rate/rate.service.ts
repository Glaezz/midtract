 ...
 */
export async function getUsdcToIdrRate(maxRetries = 2): Promise<number> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(`[CoinGecko RateFetch] Percobaan ${attempt}/${maxRetries} gagal: ${(err as Error).message}`);
  ...
  try {
    ...
    if (backupRes.ok) {
      const backupBody = (await backupRes.json()) as BackupFiatResponse;
    ...
    console.error(`[RateFetch] API Cadangan juga gagal: ${(backupErr as Error).message}`);
  }
