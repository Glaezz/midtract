...
async function callOkeconnectDisbursement(recipientNumber: string, amountIdr: bigint): Promise<{ providerPayoutId: string }> {
  if (isMockMode) {
    // Struktur respons di bawah ini ASUMSI, lihat catatan di atas.
    const mockResponse: MockOkeconnectResponse = {
      status: 'success',
      data: {
...
export async function refundToBuyer(orderId: bigint, reason: string, triggerType: 'AUTO_TIMEOUT' | 'MANUAL_ADMIN') {
  const order = await db.order.findUniqueOrThrow({ where: { id: orderId } });
