...
const PLATFORM_FEE_IDR = 1_000n; // flat, sesuai PRD
const GATEWAY_FEE_RATE = 0.007; // 0,7% dari nominal produk

function calculateFees(amountIdr: bigint) {
 ...
 */
export async function createPayment(orderCode: string) {
 ...
 */
export async function handleMidtransNotification(rawBody: unknown) {
  if (isMockMode) {
  ...
  const orderCode = statusResponse.order_id as string;
...
export async function mockConfirmPayment(orderCode: string) {
  if (!isMockMode) {
...
export async function mockExpirePayment(orderCode: string) {
  if (!isMockMode) {
