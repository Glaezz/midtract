 ...
 */
export async function joinOrderByInvite(inviteToken: string, buyerPrivyDid: string, buyerPayoutChannel: string, buyerPayoutNumber: string) {
 ...
 */
export async function listOrdersForUser(privyDid: string, role: 'seller' | 'buyer') {
 ...
 */
export async function markWaitingForSignature(orderCode: string) {
  ...
  if (claimed.count === 0) {
    ...
    console.warn(
      `[markWaitingForSignature] Order ${orderCode} sudah berstatus ${current.status}, ` +
 ...
 */
export async function confirmLock(orderCode: string, buyerWallet: string, deadline: bigint, signature: string) {
...
export async function getInviteLinkForSeller(orderCode: string, sellerPrivyDid: string) {
...
function formatUsdcAmount(amount: Prisma.Decimal, rate: Prisma.Decimal): string {
  return `${amount.toString()} USDC ($1 = Rp ${Number(rate).toLocaleString('id-ID')})`;
}

function formatIdrAmount(amountIdr: bigint): string {
 ...
 */
export async function getOrderTimeline(orderCode: string) {
  const order = await db.order.findUniqueOrThrow({
    where: { orderCode },
    include: { seller: true, buyer: true },
  });
  ...
  for (const p of payments) {
    if (p.status !== 'SUCCESS') continue; // percobaan gagal tidak perlu masuk timeline publik
    timeline.push({
      entryType: 'PAYMENT',
      title: 'Pembayaran Pembeli',
      provider: p.provider === 'midtrans' ? 'Midtrans Snap (QRIS)' : 'Simulasi Demo',
      description:
        'Pembeli melunasi tagihan QRIS. Setelah pembayaran terverifikasi, sistem melanjutkan ke proses berikutnya.',
      from: 'Pembeli (QRIS)',
      to: 'Pool Sistem',
  ...
  const onchainMeta: Record<string, { title: string; provider: string; description: string }> = {
    POOL_FUNDING: {
      title: 'Transfer USDC ke Wallet Pembeli',
      provider: 'Injective EVM Testnet',
      description:
        'Sistem mentransfer USDC dari pool wallet ke wallet pembeli, sebagai representasi nilai transaksi yang akan dikunci ke smart contract.',
    },
    LOCKED: {
      title: 'Penguncian Dana (Lock Escrow)',
      provider: 'Midtract.sol (EIP-712)',
      description:
        'Pembeli menandatangani persetujuan lewat Privy. Relayer mengeksekusi penguncian dana ke smart contract Midtract secara on-chain.',
    },
    RELEASED: {
      title: 'Pelepasan Dana ke Penjual',
      provider: 'Midtract.sol',
      description:
        'Dana dilepaskan dari smart contract ke wallet penjual, dipicu konfirmasi "Barang Diterima" dari pembeli atau tenggat auto-release yang sudah terlampaui.',
    },
    REFUNDED_ONCHAIN: {
    ...
    SWEPT_TO_POOL: {
      ...
      provider: 'Midtract.sol',
      description:
        'Sistem menarik kembali USDC ke pool wallet menggunakan izin (allowance) yang diberikan saat aktivasi akun -- tanpa perlu signature tambahan.',
    },
    RECLAIMED_TO_POOL: {
      title: 'Reclaim Top-up ke Pool',
      provider: 'Midtract.sol',
      description:
        'USDC yang sebelumnya ditransfer ke wallet pembeli ditarik kembali ke pool karena transaksi tidak dilanjutkan sampai tenggat waktu.',
    },
  };
...
export async function markInTransit(orderCode: string, courierName: string, courierReceiptNumber: string) {
  return db.order.update({
    where: { orderCode },
    data: {
      status: 'IN_TRANSIT',
...
export async function raiseDispute(orderCode: string, description: string) {
